...
#include "awa/torrent/torrenthelpers.h"
...
void TorrentService::resume(const QString& id)
{
    if (m_handles.contains(id)) {
        ...
        auto item = itemFromHandle(handle);
        if (item.state == awa::core::DownloadState::Deleted) {
            handle.pause();
            applyDeletedPayloadState(item);
            m_items.insert(id, item);
            emit itemUpdated(item);
            persistItems();
            return;
...
awa::core::DownloadItem TorrentService::itemFromHandle(const lt::torrent_handle& handle) const
{
    ...
    if (completedPayloadIsMissing(handle, status, storedSnapshot)) {
        handle.pause();
        applyDeletedPayloadState(item);
        return item;
    }
